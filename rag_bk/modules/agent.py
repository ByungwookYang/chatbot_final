...
def create_agent_executor(model_name="gpt-4o-mini", tools=[]):
    ...
    # 시스템 프롬프트 설정
    system_prompt = (
        f'{st.session_state["new_prompt"]}'  # 생성된 페르소나 프롬프트 입력
        + """You are an helpful AI Assitant like Perplexity. Your mission is to answer the user's question.

Here are the tools you can use:
{tools}
If the relevant content is available in a PDF file, please use the "pdf_retriever" tool.
If you need further information to answer the question, use the tools to get the information.

###

If you use 'web_search' tool, Please follow these instructions:
even if you do not use 'web_search', Please don't follow these instructions:
